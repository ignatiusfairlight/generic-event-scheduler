<script lang="ts">
  ...
  const form = superForm(defaults(zod4(formSchema)), {
    ...
    async onUpdate({ form: formResult }) {
      ...
      if (
        new Date(`${formResult.data.startDate} ${formResult.data.startTime}`) >
        new Date(`${formResult.data.endDate} ${formResult.data.endTime}`)
      ) {
        setError(formResult, "endTime", "End time must be after start time");
      } else {
        try {
      ...
      <Form.Field {form} name="startDate">
        <Form.Control>
          {#snippet children({ props })}
            ...
            <DatePicker {...props} bind:date={$formData.startDate} />
          {/snippet}
        </Form.Control>
        <Form.FieldErrors />
      </Form.Field>
      ...
      <Form.Field {form} name="personInCharge">
        <Form.Control>
          {#snippet children({ props })}
            ...
            <Input {...props} bind:value={$formData.personInCharge} />
          {/snippet}
        </Form.Control>
        <Form.FieldErrors />
      </Form.Field>
